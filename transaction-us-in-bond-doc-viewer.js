(() => {
  ...
  function parseRoute() {
    ...
    const match = hash.match(/^#transaction-us-in-bond\/documents\/([^/?#]+)/);
    if (!match) {
      return null;
    }
  ...
  function syncHash(row) {
    ...
    const next = `#transaction-us-in-bond/documents/${encodeURIComponent(row.id)}?cat=${encodeURIComponent(ps.docCategory)}&doc=${ps.docIndex}${viewQuery}`;
  ...
  function handleClick(event, row, helpers) {
    ...
    if (prevTxn) {
      ...
      if (prevId) {
        ...
        const viewQuery = viewQueryForMode(ps.viewerPageMode);
        helpers.goto?.(
          `#transaction-us-in-bond/documents/${encodeURIComponent(prevId)}?cat=${encodeURIComponent(ps.docCategory)}&doc=${ps.docIndex}${viewQuery}`
        );
      }
      return true;
    }
    ...
    if (nextTxn) {
      ...
      if (nextId) {
        ...
        const viewQuery = viewQueryForMode(ps.viewerPageMode);
        helpers.goto?.(
          `#transaction-us-in-bond/documents/${encodeURIComponent(nextId)}?cat=${encodeURIComponent(ps.docCategory)}&doc=${ps.docIndex}${viewQuery}`
        );
      }
      return true;
    }
