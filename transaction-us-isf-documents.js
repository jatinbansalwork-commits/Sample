(() => {
  ...
  function iconFile() {
    return window.KNIcons?.html?.("file", { size: "large" }) || "";
  }

  function iconUpload() {
    return window.KNIcons?.html?.("upload", { size: "large" }) || "";
  }

  function iconSearch() {
    return window.KNIcons?.html?.("search") || "";
  }
  function iconEye() {
    return window.KNIcons?.html?.("eye") || "";
  }

  function iconDownload() {
    return window.KNIcons?.html?.("download") || "";
  }
  ...
  function iconWarn() {
    return window.KNIcons?.html?.("warn", { size: "small", className: "kn-icon-tone-notice" }) || "";
  }
          ...
          <div class="isf-doc-hub__toolbar">
            ...
            <div class="isf-doc-hub__chips" role="tablist" aria-label="Document filters">
              ${[
                ...
                .map(
                  (chip) => `<button class="kn-btn btn ${state.filter === chip.id ? "btn--primary" : "btn--tertiary"} btn--sm type-ui-sm" type="button" role="tab" aria-selected="${state.filter === chip.id}" data-isf-doc-filter="${chip.id}">${chip.label}</button>`
                )
                .join("")}
            </div>
            ${ux().select({
